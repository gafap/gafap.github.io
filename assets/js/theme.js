...
/* LOCAL OVERRIDE of al_folio_core's assets/js/theme.js.

   The only change from the gem's version is that the theme toggle has TWO
   states (light and dark) instead of three (light, dark and system).

   The gem cycles light -> dark -> system -> light, and in the "system" state the
   navbar shows a composite half-moon/half-sun icon that is easily mistaken for
   the plain sun. Here "system" is removed entirely: light is the default, and
   the button flips straight between light and dark. The system icon is also
   hidden in CSS (see assets/css/main.scss) so it cannot appear even if this
   script fails to load.

   Three functions differ from the gem: toggleThemeSetting,
   determineThemeSetting and determineComputedTheme. Everything else is
   untouched.

   After `bundle update` of al_folio_core, compare against the gem's version:
     bundle exec al-folio upgrade overrides diff assets/js/theme.js
*/

// Toggle between the light and dark theme settings.
let toggleThemeSetting = () => {
  if (determineThemeSetting() == "light") {
...
// Default is "light". A "theme" value of "system" left in localStorage by an earlier
// version of the site falls through to the default, so visitors who had landed on the
// old third state are quietly moved back to light.
...
// Determine the computed theme, which can be "dark" or "light". With the "system"
// setting gone, the chosen setting *is* the computed theme.
...
let initTheme = () => {
  ...
  // The gem also re-applied the theme whenever the OS light/dark preference changed.
  // That listener is dropped here: the theme is now always an explicit choice, so the
  // system preference no longer feeds into it.
};
